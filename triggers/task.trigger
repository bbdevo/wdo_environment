trigger task on Task (after insert, after update, before delete) {
       ...
       trigger.isBefore && trigger.isDelete) {
           try {
               ...
               if(Limits.getQueueableJobs() == Limits.getLimitQueueableJobs()) {
                   String errMsg = 'Queuable limit reached. Task trigger failed to start queable job.';
                   utilities.publishTriggerSubError(trigger.newMap, trigger.oldMap, errMsg);
               } else {
                   subscriptionEngine engine = new subscriptionEngine(trigger.newMap, trigger.oldMap, trigger.operationType, 'Task');
                   engine.processRecords();
               }
           } catch(Exception e){
               try {
                   String errMsg = 'Task trigger failed due to ' + e.getMessage();
                   utilities.publishTriggerSubError(trigger.newMap, trigger.oldMap, errMsg);
               } catch(Exception e2){
